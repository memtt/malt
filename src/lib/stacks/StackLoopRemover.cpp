...
{
	assert(target != NULL);
	for (int i = 0 ; i < MATT_STACK_NODE_MAX_LINK ; i++)
	{
		if (links[i] == NULL)
		{
			links[i] = target;
	...
	MATT_FATAL_ARG("Not enougth link to unloop stacks, limit is %1, see macro MATT_STACK_NODE_MAX_LINK").arg(MATT_STACK_NODE_MAX_LINK).end();
}
