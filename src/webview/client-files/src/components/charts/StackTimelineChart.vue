...
import type { StackChartSeries } from '@/types/stack-peaks'
...
const tooltipId = `stack-timeline-tooltip-${Math.random().toString(36).substr(2, 9)}`
...
const renderChart = () => {
  ...
  props.data.forEach((series, index) => {
    const color = series.color || colorScale(index.toString())
    ...
    .on('mousemove', function (event) {
      ...
      const tooltipContent = props.data
        .map((series) => {
          ...
          const color = series.color || colorScale(props.data.indexOf(series).toString())

          return `<div style="margin: 2px 0;">
            <span style="color: ${color}; font-weight: bold;">${series.key}:</span>
            ${formatValue(point.y)}
          </div>`
        })
        .join('')
