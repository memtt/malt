 ...
 */
export function useAllocSizeDistrGpuHeatmaps(
  scatterData: ComputedRef<ScatterData | undefined>,
  binsX = 64,
  binsY = 64,
) {
  const sizeOverTimeGpuHeatmap = computed(() => {
    if (!scatterData.value?.sizeOverTimeGpu?.points) return []

    const points = scatterData.value.sizeOverTimeGpu.points
  ...
  const lifetimeOverSizeGpuHeatmap = computed(() => {
    if (!scatterData.value?.lifetimeOverSizeGpu?.points) return []

    const points = scatterData.value.lifetimeOverSizeGpu.points
    if (points.length === 0) return []
  ...
  return {
    sizeOverTimeGpuHeatmap,
    lifetimeOverSizeGpuHeatmap,
  }
}
